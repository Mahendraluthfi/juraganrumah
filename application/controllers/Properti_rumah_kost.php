...

	{
		if (!empty(get_cookie('user_agen'))) {
			...
			if (empty($cek->num_rows())) {
				...
				$get = $cek->row();
				$this->db->query("UPDATE stats_agen_category SET kost = kost + 1 WHERE id = '$get->id_agen'");
			}	
		}
		...
		if($this->input->post('search_beranda')){
	        ...
	        $config['base_url'] = base_url()."properti_rumah_kost/index"; //site url
	        ...
	        $data['produk'] = $this->M_buyer_produk->tampil_data_produk_kost($config["per_page"], $offset)->result();;
