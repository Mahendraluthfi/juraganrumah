...
class Register_freelance_agen extends CI_Controller {
	...
	function __construct(){
		...
		$this->load->model('M_registrasi_freelance_agen');
	...
	public function index()
	{
        ...
        $data['freelance_agen'] = $this->M_registrasi_freelance_agen->freelance_agen()->result();
		$data['content'] = 'register_freelance_agen';
    ...
    {
    	$get_email = $this->db->get_where('freelance_agen', array('id_freelance' => $id))->row();
    	$data['pt'] = $get_email->email;
        $message = $this->load->view('freelance/welcome_email', $data, true);	        
        ...
        $mail->Subject = 'Pendaftaran Agen Freelance Juragan Rumah';
...

    {
					...
					$id_freelance   = $this->input->get('id');
					...
					$data = array(
						...
						$this->M_registrasi_freelance_agen->adding($data,'project');
    ...
    {
		$nama_freelance      = $this->input->post('nama_freelance');
        if ($nama_freelance)
			{
				$nama_freelance	   = $this->input->post('nama_freelance');
				...
				$data = array(
                    ...
                    $this->M_registrasi_freelance_agen->adding($data,'freelance_agen');
                    $cek = $this->db->get_where('freelance_agen', array('email' => $email))->row();
                    $id_freelance = $cek->id_freelance;
                   	$this->send_mail($id_freelance);
					$this->project($id_freelance);
					...
					if (!empty($fotoktp)) {
		    			...
		    			$this->db->where('id_freelance', $id_freelance);
		      			$this->db->update('freelance_agen', array('file_ktp' => $this->upload->file_name));    			
					}				
					// $this->send_mail();
					$this->session->set_flashdata("sukses","Data Terkirim.<br> <a href='http://www.juraganrumah.net/freelance' class='btn btn-link'>Masuk Disini</a>");
					redirect('register_freelance_agen');
			}
			else
			{ 
