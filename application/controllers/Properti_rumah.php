...
class Properti_rumah extends CI_Controller {
...

	{
		if (!empty(get_cookie('user_agen'))) {
			...
			if (empty($cek->num_rows())) {
				...
				$get = $cek->row();
				$this->db->query("UPDATE stats_agen_category SET rumah = rumah + 1 WHERE id = '$get->id_agen'");
			}	
		}
		...
		if($this->input->post('search_beranda')){
	        ...
	        $config['base_url'] = base_url()."properti_rumah/index"; //site url

	        $config['total_rows'] = $this->db->get_where('produk', array('id_category_produk' => 1))->num_rows(); //total row
	        ...
	        $data['produk'] = $this->M_buyer_produk->tampil_data_produk_rumah($config["per_page"], $offset)->result();;
