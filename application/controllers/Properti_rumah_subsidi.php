...
class Properti_rumah_subsidi extends CI_Controller {
...

	{
		if (!empty(get_cookie('user_agen'))) {
			...
			if (empty($cek->num_rows())) {
				...
				$get = $cek->row();
				$this->db->query("UPDATE stats_agen_category SET perumahan = perumahan + 1 WHERE id = '$get->id_agen'");
			}	
		}
		...
		if($this->input->post('search_beranda')){
			...
			$data['content'] = 'properti_rumah_subsidi';
	        ...
	        $config['base_url'] = base_url()."properti_rumah_subsidi/index"; //site url
	        ...
	        $data['produk'] = $this->M_buyer_produk->tampil_data_produk_subsidi($config["per_page"], $offset)->result();;
