...
class Properti_ruko extends CI_Controller {
...

	{
		if (!empty(get_cookie('user_agen'))) {
			...
			if (empty($cek->num_rows())) {
				...
				$get = $cek->row();
				$this->db->query("UPDATE stats_agen_category SET ruko = ruko + 1 WHERE id = '$get->id_agen'");
			}	
		}
		...
		if($this->input->post('search_beranda')){
	        ...
	        $config['base_url'] = base_url()."properti_ruko/index"; //site url
	        ...
	        $data['produk'] = $this->M_buyer_produk->tampil_data_produk_ruko($config["per_page"], $offset)->result();;
