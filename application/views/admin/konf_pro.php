 ...
 <div class="page-breadcrumb">
    <div class="row">
        <div class="col-12 d-flex no-block align-items-center">
            <h4 class="page-title">Konfirmasi Mitra Developer Pro</h4>
                    ...
                    <table class="table table-bordered table-hover" id="zero_config">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php if ($data->sts == "SUBMIT"): ?>
                                        <button type="button" class="btn btn-success btn-sm margin-5" data-toggle="tooltip" data-placement="top" title="" data-original-title="Lihat Konfirmasi" onclick="konfirmasi('<?php echo $data->id_inv ?>')"><i class="fas fa-check"></i></button>
                                    <?php endif ?>

                                    <button type="button" class="btn btn-primary btn-sm margin-5" data-toggle="tooltip" data-placement="top" title="" data-original-title="Lihat Invoice" onclick="invoice('<?php echo $data->id_inv ?>')"><i class="fas fa-file-alt"></i></button>
                                    <?php if ($data->sts !== "BERHASIL"): ?>
...
<div class="modal fade" id="Modal1" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true ">
    <div class="modal-dialog" role="document ">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <?php echo form_open('admin/konf_pro/submit_tf', array('class' => 'form-horizontal')); ?>
                    <div class="card-body">                        
...
<script>
    ...
    function invoice(id) {
         $.ajax({
            url : "<?php echo site_url('index.php/admin/konf_pro/get_inv_mitra/')?>" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data)
            {   
                $('.modal-title').text('Invoice #'+data.id_inv);
                $('.font-bold').text(data.nama_perusahaan);
                $('.date').text(data.date);
                $('.identity').text(data.nama_mitra);
    ...
    function konfirmasi(id) {
        $.ajax({
            url : "<?php echo site_url('index.php/admin/konf_pro/get_mitra/')?>" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data)
            {   
